...
- **Performance:** The calendar, the selected day and Insights now fetch only the entries they show (day, month, oldest entry) through range predicates on the stored `date`, instead of four screens each loading and filtering every entry in memory [#24](https://github.com/ekomarova/Lummi-iOS/issues/24)
